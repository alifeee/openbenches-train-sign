...
echo $feed | xmlstarlet sel -T -t -v '(/rss/channel/item/description)['"${historical}"']' /dev/stdin | sed 's/<br><img src.*//g' | perl -n -mHTML::Entities -e ' ; print HTML::Entities::decode_entities($_) ;' | sed 's/^\s*//' | sed 's/\s*$//'
